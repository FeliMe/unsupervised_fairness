...
def load_rsna_naive_split(rsna_dir: str = RSNA_DIR,
    ...
    n_train = 8051 if max_train_samples is None else min(max_train_samples, 8051)
    train = normal_data.sample(n=n_train, random_state=SEED)

    # Rest for validation and test
    rest_normal = normal_data[~normal_data.patientId.isin(train.patientId)]
    val_normal = rest_normal.sample(n=400, random_state=SEED)
    test_normal = rest_normal[~rest_normal.patientId.isin(val_normal.patientId)]
    val_test = data.sample(n=800, random_state=SEED)
    val = val_test.iloc[:400, :]
    test = val_test.iloc[400:, :]

    # Concatenate and shuffle
    val = pd.concat([val_normal, val]).sample(frac=1, random_state=SEED).reset_index(drop=True)
    test = pd.concat([test_normal, test]).sample(frac=1, random_state=SEED).reset_index(drop=True)
...
def load_rsna_gender_split(rsna_dir: str = RSNA_DIR,
    ...
    # Normal
    val_test_normal_male = normal_male.sample(n=50, random_state=SEED)
    val_test_normal_female = normal_female.sample(n=50, random_state=SEED)
    val_test_male = male.sample(n=100, random_state=SEED)
    val_test_female = female.sample(n=100, random_state=SEED)
    ...
    val_male = pd.concat([val_test_normal_male, val_male]).sample(frac=1, random_state=SEED)
    val_female = pd.concat([val_test_normal_female, val_female]).sample(frac=1, random_state=SEED)
    test_male = pd.concat([val_test_normal_male, test_male]).sample(frac=1, random_state=SEED)
    test_female = pd.concat([val_test_normal_female, test_female]).sample(frac=1, random_state=SEED)
...
def load_rsna_age_two_split(rsna_dir: str = RSNA_DIR,
    ...
    # Normal
    val_test_normal_young = normal_young.sample(n=50, random_state=SEED)
    val_test_normal_old = normal_old.sample(n=50, random_state=SEED)
    val_test_young = young.sample(n=100, random_state=SEED)
    val_test_old = old.sample(n=100, random_state=SEED)
    ...
    # Aggregate validation and test sets and shuffle
    val_young = pd.concat([val_test_normal_young, val_young]).sample(frac=1, random_state=SEED).reset_index(drop=True)
    val_old = pd.concat([val_test_normal_old, val_old]).sample(frac=1, random_state=SEED).reset_index(drop=True)
    test_young = pd.concat([val_test_normal_young, test_young]).sample(frac=1, random_state=SEED).reset_index(drop=True)
    test_old = pd.concat([val_test_normal_old, test_old]).sample(frac=1, random_state=SEED).reset_index(drop=True)
    ...
    # Aggregate training set and shuffle
    train = pd.concat([train_young, train_old]).sample(frac=1, random_state=SEED).reset_index(drop=True)
